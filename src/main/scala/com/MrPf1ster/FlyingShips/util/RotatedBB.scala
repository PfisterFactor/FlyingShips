...
class RotatedBB(minPos: Vec3, maxPos: Vec3, rotationPoint: Vec3, rotationOffset: Float, yaw: Float, pitch: Float, roll: Float) {
  ...
  def updateCorners() = {
    // Actually pitch (around y-axis) due to Minecraft using a weird axis
    val radiansYaw: Float = (rotationYaw * (Math.PI / 180.0F)).toFloat
    // Actually roll (around x-axis) due to Minecraft using a weird axis
    val radiansPitch: Float = (rotationPitch * (Math.PI / 180.0f)).toFloat
    // Actually yaw ( around z-axis) due to Minecraft using a weird axis
    ...
    def properYaw(vec: Vec3, radians: Float): Vec3 = rotateVecRoll(vec, radians)
    def properPitch(vec: Vec3, radians: Float): Vec3 = vec.rotateYaw(radians)
    def properRoll(vec: Vec3, radians: Float): Vec3 = vec.rotatePitch(radians)

    for (i <- Corners.indices) {
      Corners(i) = Corners(i).subtract(RotationPoint)

      var rotatedCorner = properRoll(Corners(i), radiansIdontknow)
      rotatedCorner = properYaw(rotatedCorner, radiansOffset)
      rotatedCorner = properPitch(rotatedCorner, radiansIdontknow)





      rotatedCorner = properPitch(rotatedCorner, radiansPitch)
      rotatedCorner = properYaw(rotatedCorner, radiansYaw)
      rotatedCorner = properRoll(rotatedCorner, radiansRoll)

      rotatedCorner = properRoll(rotatedCorner, -radiansIdontknow)
      rotatedCorner = properYaw(rotatedCorner, -radiansOffset)
      rotatedCorner = properPitch(rotatedCorner, -radiansIdontknow)
